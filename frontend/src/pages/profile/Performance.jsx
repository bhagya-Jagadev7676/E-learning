...
const Performance = () => {
  ...
  useEffect(() => {
    const fetchPerformanceData = async () => {
      ...
      if (result.success) {
        ...
        const data = Array.isArray(result.data) ? result.data : (result.data.assessments || []);
  ...
  const calculateStats = () => {
    ...
    const completed = performanceData.filter(data => {
      const marks = data.marks || (data.score / data.total_questions * 100);
      return marks > 0;
    }).length;
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {performanceData.map((data, index) => {
                    ...
                    const marks = data.marks || (data.score / data.total_questions * 100);
                    const courseName = data.course?.course_name || data.course_title || 'Unknown Course';
                    const courseId = data.course?.course_id || data.course_id || index;
                    
                    const scoreData = getScoreIcon(marks);
                    return (
                      <tr
                        key={index}
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-center">
                          {marks > 0 ? (
                            <div className="flex items-center justify-center gap-2">
                              <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-bold ${getScoreColor(marks)}`}>
